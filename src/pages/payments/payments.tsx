...
function Payments() {
  ...
  const categories = useMemo(() => {
    const categoryList = data?.services
      ? [...new Set(data.services.map((service) => service.category_code))].map((categoryCode) => ({
          name: data.services.filter((service) => service.category_code === categoryCode)[0]
            .category_name,
          code: categoryCode,
        }))
      : [];
  ...
  useEffect(() => {
    setSelectedCategory((prev) => {
      const matchedCategory = categories.filter((category) => category.code === prev.code);
  ...
  useEffect(() => {
    if (categories.length > 0) {
      setSelectedCategory({
        name: search.length > 0 ? 'Search results: ' + search : categories[0].name,
              ...
              {data?.services
                .filter(service =>
                  selectedCategory.code === 'search' && search.length > 0
                    ...
                    : selectedCategory.code === categorySaved[userLanguage || ''].code
                    ? service.saved
